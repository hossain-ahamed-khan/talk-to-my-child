...
import ParentVoiceChatModal from "@/components/parent/ParentVoiceChatModal";
        ...
        <div className="relative flex min-h-[calc(100vh-60px)] flex-col overflow-hidden bg-[#0f172a] text-white" style={{ fontFamily: "'DM Sans', 'Segoe UI', sans-serif" }}>
            ...
            <div className="relative z-10 flex min-h-0 flex-1">
                <aside className="hidden w-64 shrink-0 border-r border-[#1e2d3e] bg-[#111c2b]/80 p-4 md:flex md:flex-col">
                    <div className="mb-4 flex items-center justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#4ade80]">Conversations</p><p className="mt-1 text-xs text-[#6b7a8d]">{sessions.length} session{sessions.length === 1 ? "" : "s"}</p></div><button onClick={() => void createSession()} disabled={isCreating || !token} title="New conversation" className="cursor-pointer rounded-lg p-2 text-[#8b9ab0] transition hover:bg-[#1b4648] hover:text-white disabled:opacity-50">{isCreating ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />}</button></div>
                    <div className="min-h-0 flex-1 space-y-1 overflow-y-auto">{sessions.map((session) => <div key={session.id} className={`group flex items-center rounded-xl transition ${activeSession?.id === session.id ? "bg-[#1b4648]" : "hover:bg-white/5"}`}>
                        {editingSessionId === session.id ? <form onSubmit={(event) => { event.preventDefault(); void renameSession(session); }} className="flex min-w-0 flex-1 items-center gap-2 px-3 py-2">
                            ...
                            <button type="submit" disabled={updatingSessionId === session.id} title="Save conversation name" className="cursor-pointer rounded p-1 text-[#4ade80] hover:bg-white/10 disabled:opacity-50"><Check className="h-4 w-4" /></button>
                            <button type="button" onClick={() => setEditingSessionId(null)} title="Cancel rename" className="cursor-pointer rounded p-1 text-[#8b9ab0] hover:bg-white/10 hover:text-white"><X className="h-4 w-4" /></button>
                        </form> : <>
                            <button onClick={() => connectToSession(session)} className="cursor-pointer min-w-0 flex-1 px-3 py-3 text-left"><div className="flex items-center gap-2"><MessageCircle className="h-4 w-4 shrink-0 text-[#4ade80]" /><span className="truncate text-sm font-medium text-[#d8e2ec]">{session.title || "New conversation"}</span></div><p className="mt-1 pl-6 text-[11px] text-[#6b7a8d]">{session.message_count} message{session.message_count === 1 ? "" : "s"}</p></button>
                            <div className="mr-2 hidden items-center gap-1 group-hover:flex"><button onClick={() => { setEditingSessionId(session.id); setSessionTitle(session.title || ""); }} title="Rename conversation" className="cursor-pointer rounded p-1.5 text-[#8b9ab0] hover:bg-white/10 hover:text-white"><Pencil className="h-3.5 w-3.5" /></button><button onClick={() => void deleteSession(session)} disabled={updatingSessionId === session.id} title="Delete conversation" className="cursor-pointer rounded p-1.5 text-[#8b9ab0] hover:bg-red-500/20 hover:text-red-300 disabled:opacity-50"><Trash2 className="h-3.5 w-3.5" /></button></div>
                        </>}
                    </div>)}</div>
                ...
                <section className="flex min-w-0 flex-1 flex-col">
                    ...
                    <div className="px-5 pb-5 pt-3 md:px-8">{error && <div className="mx-auto mb-3 flex max-w-3xl items-center justify-between rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs text-red-200"><span>{error}</span><button onClick={() => setError(null)} title="Dismiss error" className="cursor-pointer"><X className="h-4 w-4" /></button></div>}<form onSubmit={sendMessage} className="mx-auto flex max-w-3xl items-center gap-2 rounded-full border border-white/10 bg-white/5 p-2 pl-3 backdrop-blur-xl focus-within:border-emerald-400/40"><button type="button" title="Attach a file" className="cursor-pointer rounded-full p-2 text-white/40 transition hover:bg-white/10 hover:text-white/80"><Paperclip className="h-4 w-4" /></button><input value={message} onChange={(event) => setMessage(event.target.value)} disabled={!activeSession || connection !== "connected" || isSending} placeholder={activeSession ? "Message..." : "Select a conversation..."} className="min-w-0 flex-1 bg-transparent text-sm text-[#c5d1de] outline-none placeholder:text-white/30 disabled:cursor-not-allowed" /><button type="submit" disabled={!message.trim() || !activeSession || connection !== "connected" || isSending} title="Send message" className="cursor-pointer flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-emerald-600 transition hover:bg-emerald-500 disabled:cursor-default disabled:bg-white/10"><Send className="h-4 w-4" /></button><button type="button" onClick={() => { if (!token) { toast.error("Please sign in to start a voice chat."); return; } setIsVoiceChatOpen(true); }} title="Start voice chat" className="cursor-pointer flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-white/10 transition hover:bg-white/20"><Mic className="h-4 w-4" /></button></form></div>
                </section>
            </div>
            {isVoiceChatOpen && token && <ParentVoiceChatModal token={token} onClose={() => setIsVoiceChatOpen(false)} />}
        </div>
    );
}
